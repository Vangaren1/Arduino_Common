...
SoilSensor::SoilSensor(int input)
{
    bool pinCheck = PinManager::reservePin(input);
...
bool SoilSensor::validConfiguration() const
{
    return validConfig;
}
...
#if defined(ESP32)
        dryValue = 3000; // likely “pretty dry”
        wetValue = 1200; // likely “quite wet”
#else
...
void SoilSensor::setCalibration(int dry, int wet)
{
    ...
    if (wet == dry)
        return; // avoid divide-by-zero later
...
int SoilSensor::readPercent() const
{
    ...
    if (dryVal < 0 || wetVal < 0 || dryVal == wetVal)
        return -1;
